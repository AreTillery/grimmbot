...
async def listspeakers(ctx):
    ...
    if len(outstr):
        await client.send_message(ctx.message.channel, outstr)
    else:
        await client.send_message(ctx.message.channel, "No speakers are in the list; this might be a bug.")
...
async def speaker(ctx, whichspeaker):
    ...
    if bynum:
        ...
        if which > len(speakers):
            await client.send_message(ctx.message.channel, "Invalid speaker index. Max value is {}".format(len(speakers) + 1))
            return
        if which < 0:
            await client.send_message(ctx.message.channel, "Invalid speaker index. The list starts at 1.")
            return
        # return the info
        ...
        if len(matches) == 0:
            await client.send_message(ctx.message.channel, "Invalid speaker. Get the full list using !listspeakers")
            return
        bio = matches[0]["bio"]
        name = matches[0]["name"]
    await client.send_message(ctx.message.channel, f"{name}:\n{bio}")
...
async def listtalks(ctx, track=None):
    ...
    if track is None:
        ...
        await client.send_message(ctx.message.channel, "Invalid track. Options are: one, two")
        return
    ...
    outstr = ""
    if len(alltalks) == 0:
        await client.send_message(ctx.message.channel, "There are no talks in the schedule. This might be a bug.")
        return
...
async def talk(ctx, name_or_time, track=None):
    ...
    if len(talks_at_time) == 0:
        await client.send_message(ctx.message.channel, "No talks matched your query.")
        return
...
async def addtalk(ctx, talktitle, speakername, time, track, speakerbio=None):
    ...
    if track in ["1", "one", "trackone"]:
        ...
        await client.send_message(ctx.message.channel, "You must specify a track. Valid tracks: one, two")
        return
    ...
    if len(alltalks):
        ...
        if len(matching):
            await client.send_message(ctx.message.channel, "A talk with that title exists. Modify it with !modifytalk \"{}\" \"{}\" (and optionally also set the time".format(talktitle, speakername))
            return
    ...
    if len(matches) == 0:
        if speakerbio is None:
            await client.send_message(ctx.message.channel, "No speaker by that name exists. To create a new one, also supply a bio:\n!addtalk \"{}\" \"{}\" \"{}\" \"{}\" \"speaker bio\"".format(talktitle, speakername, time, track))
            return
    ...
    await client.send_message(ctx.message.channel, "Added.")
    saveTalks()
...
async def addspeaker(ctx, name, bio):
    ...
    matches = [speaker for speaker in speakers if speaker["name"].lower() == which.lower()]
    if len(matches):
        await client.send_message(ctx.message.channel, "That speaker already exists. To modify:\n!modifyspeaker \"{}\" \"speaker bio\"".format(name))
        return
    addspeaker(name, bio)
    await client.send_message(ctx.message.channel, "Added.")
...
async def modifytalk(ctx, talk, newtitle, speaker=None, time=None, newtrack=None):
    ...
    print(matching)
    if len(matching) == 0:
        await client.send_message(ctx.message.channel, "No talk with that title exists. Create it with !addtalk \"{}\" \"{}\" (and optionally also set the time".format(newtitle, speaker))
        return
    ...
    if newtrack is not None:
        ...
        if newtrack in ["1", "one", "trackone"]:
            ...
            await client.send_message(ctx.message.channel, "Invalid track. Options are: one, two. Other modifications were still made")
            return
        talk["track"] = newtrack
...
async def modifyspeaker(ctx, speaker, newname=None, newbio=None):
    ...
    if newname is None:
        await client.send_message(ctx.message.channel, "You must specify something to modify.\n!modifyspeaker \"{}\" \"new name\" \"new bio\"".format(speaker))
        return
